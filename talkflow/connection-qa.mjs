...
try{
  ...
  await page.route("**/api/talkflow/models/",async route=>{requests.models++;requests.pending++;try{await route.fulfill({status:200,contentType:"application/json",body:JSON.stringify({data:[{id:"claude-sonnet-4-6",display_name:"Claude Sonnet 4.6",created_at:"2026-02-17T00:00:00Z"}]})})}finally{requests.pending--}});
  await page.route("**/api/talkflow/messages/",async route=>{requests.messages++;requests.pending++;requests.body=route.request().postDataJSON();try{await new Promise(resolve=>setTimeout(resolve,80));await route.fulfill(failMessages?{status:401,contentType:"application/json",body:JSON.stringify({error:{type:"authentication_error",message:"AI 서버 인증을 확인할 수 없습니다."}})}:{status:200,contentType:"application/json",body:JSON.stringify({content:[{type:"text",text:"OK"}]})})}finally{requests.pending--}});
  ...
  check("short non-streaming request",requests.body?.stream===undefined&&requests.body?.max_tokens===8,JSON.stringify({stream:requests.body?.stream,maxTokens:requests.body?.max_tokens}));
  ...
  check("failure shows redacted server guidance",await page.getByText("AI 서버에 연결할 수 없습니다.",{exact:true}).count()===1);
