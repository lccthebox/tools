...
try {
  ...
  check("topic preparation is default", await page.locator("#calendar-view .preparation-list").isVisible());
  check("operating dates render as rows", await page.locator(".preparation-row").count() >= 8);
  ...
  check("8 approved sample topics", await page.locator(".preparation-row .status-pill.approved").count() === 8);
  await page.locator(".preparation-row [data-open$=':student']").first().click();
  ...
  for (const [width,height] of sizes) {
    ...
    await page.getByRole("button", { name: "토픽 준비", exact: true }).click();
