(function(root){
  ...
  function validateContent(topic,plan){
    ...
    const storyName=clean(plan?.star).split(/\s+/).at(-1)?.toLowerCase();if(storyName&&!allText.toLowerCase().includes(storyName))issues.push(issue("page1.story.en","승인된 STAR 이름이 Story에 필요합니다."));
    const storyWords=allText.split(/\s+/).filter(Boolean).length;if(storyWords<55||storyWords>75)issues.push(issue("page1.story.en","Story 영어는 55~75단어여야 합니다."));
