(function () {
  ...
  let legacyApiKeyPresent=false,aiServerState={checked:false,configured:false,authenticated:false,model:"Claude Sonnet 4.6"};
  ...
  function loadTopics(){
    try{
      ...
      if(parsed&&typeof parsed==="object"&&!Array.isArray(parsed)){
        const interruptedAt=new Date().toISOString(),staleBefore=Date.now()-5*60*1000;let recovered=false;
        Object.values(parsed).forEach(topic=>{const timestamp=Date.parse(topic?.updatedAt||topic?.createdAt||"");if(topic?.operatorStatus?.generationStatus!=="running"||Number.isFinite(timestamp)&&timestamp>staleBefore)return;topic.operatorStatus={...topic.operatorStatus,generationStatus:"interrupted"};topic.status="generation_interrupted";topic.failedSection=topic.failedSection||"plan";topic.generationFailure={stage:topic.failedSection,section:topic.failedSection,message:"이전 생성 작업이 완료되지 않았습니다.",issues:[],error:{type:"generation_interrupted",stage:topic.failedSection,lastAttemptAt:topic.updatedAt||topic.createdAt||interruptedAt}};topic.quality={status:"review",score:0,issues:["이전 생성 작업이 완료되지 않았습니다."]};topic.updatedAt=interruptedAt;recovered=true});
        if(recovered)localStorage.setItem(KEYS.data,JSON.stringify(parsed));
  ...
  function loadSettings(){try{const value=JSON.parse(localStorage.getItem(KEYS.settings)||"{}");legacyApiKeyPresent=typeof value.apiKey==="string"&&Boolean(value.apiKey.trim());delete value.apiKey;return value}catch{legacyApiKeyPresent=false;return{}}}
  ...
  async function preflightModel({signal}={}){
    if(!aiReady()){const error=new Error(aiServerState.configured?"관리자 로그인이 필요합니다.":"AI 서버 설정이 필요합니다.");error.stage="plan";error.type=aiServerState.configured?"login_required":"server_not_configured";throw error}
...
function canPreviewTopic(topic){if(!topic||["running","failed","interrupted"].includes(topic.operatorStatus?.generationStatus))return false;return!topic.generatedConversation||topic.generationEngine===TopicV4.VERSION&&TopicV4.evaluate(topic).ready||topic.generationEngine===Simple.VERSION&&Simple.evaluate(topic,Object.values(topics)).ready||topic.generationEngine===Generation.VERSION&&Generation.evaluate(topic,Object.values(topics)).ready}
  ...
  function lifecycleState(topic){
    ...
    return{key:"review",label:"검수 필요",action:"검수하기"};
  }
  ...
  function render(){
    ...
    renderList();
    $(".app-shell").classList.toggle("wide-mode",view==="prepare"||view==="print");
    $("#calendar-view").innerHTML=view==="prepare"?renderPreparationWorkspace():"";
    $("#topic-view").innerHTML=view==="topic"?renderTopicView(current()):"";
    $("#admin-view").innerHTML=view==="admin"?renderAdmin(current()):"";
    $("#print-view").innerHTML=view==="print"?renderPrintCollection():"";
    $("#calendar-view").hidden=view!=="prepare";
    $("#topic-view").hidden=view!=="topic";$("#admin-view").hidden=view!=="admin";
    $("#print-view").hidden=view!=="print";
    document.querySelectorAll(".tab").forEach(button=>{const active=button.dataset.view==="prepare"&&view==="prepare";button.classList.toggle("is-active",active);button.setAttribute("aria-current",active?"page":"false")});
    updateDocumentTitle();
    bindDynamic();
  }
  function monthValue(){return monthPrefix()}
  function routeFromState(){
    if(view==="print")return{section:"print",date:activeDate};
    if($("#settings-dialog")?.open)return{section:"settings",panel:settingsPanel};
    const route={section:"topics",view:prepareMode,month:monthValue()};
    const date=drawerDate||(["admin","topic"].includes(view)?activeDate:"");
    if(date){route.date=date;route.tab=view==="topic"?"topic":"review";if(route.tab==="topic")route.page=previewPage}
    return route;
  }
  function normalizeRoute(input){
    const nowMonth=monthValue(),route={section:["topics","print","settings"].includes(input.section)?input.section:"topics"};
    if(route.section==="print"){if(input.date&&topics[input.date]){route.date=input.date;return route}return{section:"topics",view:"tasks",month:nowMonth}}
    ...
    if(validDate){route.date=input.date;route.tab=input.tab==="review"?"review":"topic";if(route.tab==="topic")route.page=["1","2"].includes(String(input.page))?String(input.page):"1"}
    return route;
  }
  function parseRouteFromUrl(){const params=new URLSearchParams(location.search),legacy=params.get("view");return normalizeRoute({section:params.get("section")||(legacy==="batch"||legacy==="print"?"print":"topics"),view:legacy||"tasks",month:params.get("month"),date:params.get("date"),tab:params.get("tab")||({admin:"review",student:"student",leader:"leader"}[legacy]),page:params.get("page"),panel:params.get("panel"),role:params.get("role")})}
  function routeUrl(route){const url=new URL(location.pathname,location.origin),params=url.searchParams;params.set("section",route.section);if(route.section==="topics"){params.set("view",route.view);params.set("month",route.month);if(route.date){params.set("date",route.date);params.set("tab",route.tab);if(route.page)params.set("page",route.page)}}else if(route.section==="settings")params.set("panel",route.panel);else if(route.date)params.set("date",route.date);return url}
  function applyRoute(route){
    ...
    if(normalized.section==="topics"){
      ...
      if(normalized.date){activeDate=normalized.date;if(normalized.tab==="topic")view="topic";else{view="prepare";drawerDate=normalized.date}}
      else view="prepare";
    }else if(normalized.section==="print"){drawerDate="";activeDate=normalized.date;printDates=new Set([activeDate]);view="print"}else{settingsPanel=normalized.panel;view="prepare";drawerDate=""}
  ...
  function notifyRoute(route){const labels={tasks:"해야 할 일",month:"전체 월",calendar:"달력",topic:"오늘의 토픽",review:"내용 검수"};notify(route.section==="print"?"토픽 인쇄":route.section==="settings"?"설정":labels[route.tab]||labels[route.view]||"토픽 준비")}
  function updateDocumentTitle(){const topic=topics[activeDate],month=`${cursor.getMonth()+1}월`,singlePrint=view==="print"&&topic,section=new URLSearchParams(location.search).get("section"),label=section==="settings"?"설정":view==="topic"?`${topic?.title?.ko||"토픽"} · 오늘의 토픽`:singlePrint?`${topic.date}_TheBox_Topic_${topic.title?.ko||topic.title?.en||"topic"}`:drawerDate?`${topic?.title?.ko||drawerDate} · 내용 검수`:prepareMode==="month"?`${month} 전체 월`:prepareMode==="calendar"?`${month} 달력`:"관리자";document.title=`${label} · TheBox Topic`}
  function renderList(){
  ...
  function preparationDates(){return [...new Set([...operatingDates(),...Object.keys(topics).filter(date=>date.startsWith(monthPrefix()))])].sort()}
  function primaryAction(date,topic){const state=lifecycleState(topic),labels={"generation-failed":"문제 해결",review:"확인하기","print-ready":"승인하기",empty:"토픽 만들기",approved:"토픽 보기",invalid:"이전 형식 보기",draft:"진행 보기"};if(!topic)return `<button class="button primary" data-custom-date="${date}">${labels.empty}</button>`;if(topic.operatorStatus?.generationStatus==="interrupted")return `<button class="button primary" data-action="regenerate-v2">다시 생성</button><button class="button secondary" data-open="${date}:admin">초안 보기</button>`;const target=state.key==="approved"?"topic":"admin";return `<button class="button ${state.key==="approved"?"secondary":"primary"}" data-open="${date}:${target}">${labels[state.key]||"확인하기"}</button>`}
  function preparationRow(date,className="task-row"){const topic=topics[date],state=lifecycleState(topic);return `<article class="${className} ${state.key}" data-drawer-date="${date}" tabindex="0"><time><b>${Number(date.slice(5,7))}/${Number(date.slice(8))}</b><span>${weekday(date)}</span></time><div class="preparation-topic"><strong>${topic?koTitle(topic.title?.ko||topic.title?.en):"아직 토픽이 없습니다"}</strong></div><span class="status-pill ${state.key}">${state.label}</span><div class="row-primary">${primaryAction(date,topic)}</div></article>`}
  function taskPriority(date){const state=lifecycleState(topics[date]),rank={"generation-failed":0,review:1,"print-ready":2,empty:4,draft:1,invalid:5,approved:9}[state.key]??8,todayTime=new Date(`${today()}T12:00:00`).getTime(),distance=Math.abs(new Date(`${date}T12:00:00`).getTime()-todayTime);return[rank,distance,date]}
  function comparePriority(a,b){const left=taskPriority(a),right=taskPriority(b);return left[0]-right[0]||left[1]-right[1]||left[2].localeCompare(right[2])}
  function weekDates(){const now=new Date(`${today()}T12:00:00`),start=new Date(now);start.setDate(now.getDate()-((now.getDay()+6)%7));const end=new Date(start);end.setDate(start.getDate()+7);return operatingDates().filter(date=>{const value=new Date(`${date}T12:00:00`);return value>=start&&value<end})}
  function renderPrepareViews(includeHome=false){return `<nav class="workspace-views" aria-label="토픽 준비 화면">${includeHome?`<button data-prepare-mode="tasks">토픽 준비</button>`:""}<button class="${prepareMode==="month"?"is-active":""}" data-prepare-mode="month" aria-selected="${prepareMode==="month"}">전체 월 보기</button><button class="${prepareMode==="calendar"?"is-active":""}" data-prepare-mode="calendar" aria-selected="${prepareMode==="calendar"}">달력 보기</button></nav>`}
  function weekStatus(date){const state=lifecycleState(topics[date]),complete=state.key==="approved";return `<div class="week-status ${state.key}"><time>${weekday(date)}</time><span aria-hidden="true"></span><strong>${complete?"준비 완료":state.label}</strong></div>`}
  function renderTaskHome(){const all=operatingDates(),actionable=all.filter(date=>lifecycleState(topics[date]).key!=="approved").sort(comparePriority),shown=actionable.slice(0,8),missing=all.filter(date=>!topics[date]),week=weekDates(),weekDone=week.length&&week.every(date=>lifecycleState(topics[date]).key==="approved");return `<div class="task-home"><section class="task-section"><header><div><p class="eyebrow">ACTION</p><h2>지금 확인할 것 <span>${actionable.length}건</span></h2></div></header><div class="task-list">${shown.length?shown.map(date=>preparationRow(date)).join(""):`<p class="empty-task">지금 확인할 항목이 없습니다.</p>`}</div>${missing.length>shown.filter(date=>!topics[date]).length?`<button class="text-action" data-show-missing>미작성 ${missing.length}건 전체 월에서 보기</button>`:""}</section><section class="this-week"><header><div><p class="eyebrow">THIS WEEK</p><h2>이번 주</h2></div></header>${weekDone?`<p class="week-complete">이번 주 토픽 준비가 모두 끝났습니다.</p>`:`<div class="week-status-list">${week.length?week.map(weekStatus).join(""):`<p class="empty-task">이번 주 운영일 없음</p>`}</div>`}</section>${renderPrepareViews()}</div>`}
  function weekLabel(dates){const first=Number(dates[0].slice(8)),last=Number(dates.at(-1).slice(8));return`${cursor.getMonth()+1}월 ${first}일–${last}일`}
  function renderMonthList(){const filtered=operatingDates().filter(date=>{const key=lifecycleState(topics[date]).key;if(prepareFilter==="attention")return["generation-failed","review","print-ready","draft"].includes(key);if(prepareFilter==="missing")return key==="empty";if(prepareFilter==="approved")return key==="approved";if(prepareFilter==="legacy")return key==="invalid";return true}),groups=[];filtered.forEach(date=>{const value=new Date(`${date}T12:00:00`),week=Math.floor((value.getDate()+((new Date(`${monthPrefix()}-01T12:00:00`).getDay()+6)%7)-1)/7);(groups[week]??=[]).push(date)});return `<section class="compact-month"><div class="month-filters" aria-label="월 보기 필터">${[["all","전체"],["attention","확인 필요"],["missing","미작성"],["approved","승인 완료"],["legacy","이전 형식"]].map(([key,label])=>`<button class="${prepareFilter===key?"is-active":""}" data-month-filter="${key}">${label}</button>`).join("")}</div>${groups.filter(Boolean).map(dates=>`<section class="week-group"><h2>${weekLabel(dates)}</h2><div>${dates.map(date=>preparationRow(date,"month-row")).join("")}</div></section>`).join("")||'<p class="empty-task">조건에 맞는 운영일이 없습니다.</p>'}</section>`}
  ...
  function renderDateDrawer(){if(!drawerDate)return"";const topic=topics[drawerDate],state=lifecycleState(topic);return `<div class="drawer-backdrop" data-drawer-close></div><aside class="date-drawer" role="dialog" aria-modal="true" aria-labelledby="drawer-title"><header><button class="drawer-back" data-drawer-close>← ${cursor.getMonth()+1}월 전체 월</button><button class="icon-button" data-drawer-close aria-label="닫기">×</button></header><div class="drawer-title"><span>${drawerDate} · ${weekday(drawerDate)}</span><h2 id="drawer-title">${topic?koTitle(topic.title?.ko||topic.title?.en):"아직 토픽이 없습니다"}</h2><span class="status-pill ${state.key}">${state.label}</span></div>${topic?`<nav class="drawer-tabs" aria-label="토픽 화면"><button class="is-active" data-open="${drawerDate}:admin">내용 검수</button><button data-open="${drawerDate}:topic" ${canPreviewTopic(topic)?"":"disabled"}>오늘의 토픽</button></nav><section class="drawer-summary"><h3>${topic.operatorStatus?.generationStatus==="interrupted"?"생성이 중단되었습니다.":state.key==="generation-failed"?"생성 문제를 해결해 주세요":state.key==="approved"?"멤버에게 보여줄 토픽이 준비됐습니다.":"내용을 확인해 주세요"}</h3><p>${topic.operatorStatus?.generationStatus==="interrupted"?"이전 생성 작업이 완료되지 않았습니다. 작성 중이던 상태는 보존되어 있습니다.":state.key==="generation-failed"?"실패한 부분만 다시 생성하거나 AI 설정을 확인할 수 있습니다.":state.key==="approved"?"오늘의 토픽 한 화면과 인쇄 자료가 같은 승인 데이터를 사용합니다.":"확인 항목을 검토한 뒤 승인할 수 있습니다."}</p>${primaryAction(drawerDate,topic)}</section>`:`<section class="drawer-create"><h3>토픽을 준비하세요</h3><p>최근 주제와 이번 달 구성을 보고 새로운 토픽을 자동으로 만듭니다.</p><button class="button primary" data-auto-date="${drawerDate}">자동으로 만들기</button><span>직접 원하는 주제가 있나요?</span><button class="button secondary" data-guided-date="${drawerDate}">직접 주제 지정</button></section>`}</aside>`}
  function renderPreparationWorkspace(){const dates=operatingDates(),counts=dates.reduce((acc,date)=>{const key=lifecycleState(topics[date]).key;acc[key]=(acc[key]||0)+1;return acc},{}),attention=dates.length-(counts.approved||0)-(counts.empty||0);return `<header class="workspace-heading compact-heading"><div class="product-intro"><p class="eyebrow">THEBOX TALK</p><h1>스터디 토픽을 만들고 준비하는 공간</h1></div><div class="month-title"><div><h2>${cursor.getFullYear()}년 ${cursor.getMonth()+1}월</h2><p>승인 ${counts.approved||0} · 확인 필요 ${attention} · 미작성 ${counts.empty||0}</p></div><div class="workspace-month"><button class="icon-button" data-action="previous-month" aria-label="이전 달">←</button><button class="text-action" data-action="today">이번 달</button><button class="icon-button" data-action="next-month" aria-label="다음 달">→</button></div></div>${prepareMode==="tasks"?"":renderPrepareViews(true)}</header>${prepareMode==="tasks"?renderTaskHome():prepareMode==="month"?renderMonthList():renderCompactCalendar()}${renderDateDrawer()}`}
  ...
  function renderCalendar(){
    ...
    return `<header class="calendar-toolbar"><button class="icon-button" data-action="previous-month" aria-label="이전 달">←</button><div><h1>${year}년 ${month+1}월</h1><p>${operationDays.size}개 운영일 · ${targets.length}개 미작성</p></div><button class="icon-button" data-action="next-month" aria-label="다음 달">→</button><div class="calendar-actions"><button class="button secondary" data-action="today">오늘로 이동</button><button class="button primary" data-action="auto-compose-month">이번 달 자동 구성</button><button class="button secondary" data-action="settings">고급 설정</button></div></header>
      ...
      <section class="month-calendar">${Array.from({length:offset},()=>'<span class="calendar-spacer"></span>').join("")}${Array.from({length:days},(_,i)=>{
        ...
        return `<article class="calendar-day is-complete ${t.hidden?"is-hidden":""}"><header><time>${i+1}일 · ${weekday(date)}</time><span class="status-pill ${state}">${label}</span></header><h2>${koTitle(t.title.ko||t.title.en)}</h2><div class="date-actions"><button class="button ${state==="print-ready"?"primary":"secondary"}" data-open="${date}:${primaryTarget}">${status.action}</button><details class="card-overflow"><summary aria-label="추가 작업">⋯</summary><div><button data-open="${date}:admin">수정</button>${state==="print-ready"?`<button data-used="${date}">사용 완료로 표시</button>`:""}<button data-toggle-hidden="${date}">${t.hidden?"공개":"숨김"}</button><button data-clone-from="${date}">복제</button></div></details></div></article>`;
  ...
  function renderOperatorWorkflow(){
    ...
    return `<section class="operator-workflow" aria-label="토픽 제작 5단계">
      ...
      <div class="operator-actions create-actions"><span><button class="button secondary" data-action="recommend-topic">추천 주제 넣기</button><small>최근 사용하지 않은 주제를 입력합니다.</small></span><span><button class="button primary" data-action="operator-create" disabled>토픽 생성하기</button><small>입력한 주제로 오늘의 토픽을 만듭니다.</small></span>${canPreviewTopic(selected)?`<span class="preview-action"><button class="button secondary" data-open="${selected.date}:print">토픽 A4 미리보기</button></span>`:""}</div>
      <div class="operator-status ${review?.status||"pending"}"><strong>${review?review.status==="ready"?"바로 사용 가능":review.status==="review"?"확인할 부분 있음":"다시 생성 권장":"날짜와 주제를 선택해 시작하세요."}</strong><span>${review?`${review.issues.length}개 확인 항목 · ${review.issues[0]||"토픽 A4 미리보기 후 승인하세요."}`:"추천 주제를 넣거나 직접 입력하면 생성 버튼이 활성화됩니다."}</span></div>
    </section>`;
  }
  ...
  function evaluateRenderedPrint(t){
    ...
    if(pages.length!==expectedPages)issues.push(`A4 페이지가 ${pages.length}개입니다.`);
  ...
  function printHeader(t,page,leader=false){
    return `<header class="handout-header"><div class="handout-brand"><strong>THEBOX</strong><span>TOPIC</span></div><div class="handout-title"><span>${esc(t.date)} · ${weekday(t.date)}</span><h1>${esc(t.title.en)}</h1><p>${esc(t.title.ko)}</p></div><div class="handout-label">TODAY'S TOPIC<b>Page ${page} / 2</b></div></header>`;
  }
  ...
  function generatedHeader(t,leader,page){return `<header class="bound-header"><div class="bound-brand"><b>THEBOX</b><span>TOPIC</span></div><div class="bound-meta">${esc(t.date)} · ${esc(t.weekday)} <em>|</em> ${esc(t.category.en)} · ${esc(t.category.ko)}</div><div class="bound-role">TOPIC · ${page} / 2</div></header>`}
  ...
  function simpleHeader(t,leader,page,total=2,label=""){return `<header class="simple-header"><b>THEBOX · TOPIC</b><span>${esc(t.date)} · ${esc(t.category.en)} · CORE</span><em>${esc(label||`TOPIC ${page} / ${total}`)}</em></header><div class="simple-title"><h1>${esc(t.title.en)}</h1><p lang="ko">${esc(t.title.ko)}</p></div>`}
  function simpleLeader(note){return `<aside class="simple-leader-note"><b>LEADER</b><span>${esc(note)}</span></aside>`}
  function simpleQuestions(items,showKorean=true){return `<ol class="simple-questions">${items.map(item=>`<li><strong>${esc(item.en)}</strong>${showKorean?`<span>${esc(item.ko)}</span>`:""}${item.options?.length?`<p class="simple-options">${item.options.map(option=>`□ ${esc(option)}`).join(" · ")}</p>`:""}${item.example?`<p class="simple-help">${esc(item.example)}</p>`:""}</li>`).join("")}</ol>`}
  function selfRoundTitle(label,round){return `<h2 class="simple-round-title"><span>${esc(label)}</span><small>${round.number} · ${round.minutes} MIN · HOST #${round.host}</small></h2>${round.formatEn?`<p class="simple-format-en">${esc(round.formatEn)}</p>`:""}${round.instructionKo?`<p class="simple-round-instruction-ko" lang="ko">${esc(round.instructionKo)}</p>`:""}`}
  function simpleKeyWords(t){const text=[...(t.session1?.story?.en||[]),...(t.session1?.easyTalk||[]).map(item=>item.en),...(t.session1?.realTalk||[]).map(item=>item.en),...(t.session2?.activity?.materials||[]).map(item=>item.en)].join(" ").toLowerCase(),dictionary={crowded:"붐비는",calm:"조용한",regret:"후회하다",trust:"신뢰하다",misleading:"오해하게 하는",reliable:"믿을 만한",ordinary:"평범한",compromise:"타협하다"},words=Object.keys(dictionary).filter(word=>text.includes(word)).slice(0,5);return `<section class="simple-section simple-key-words"><b>KEY WORDS</b><p>${words.map(word=>`<span><strong>${esc(word)}</strong> <em lang="ko">${esc(dictionary[word])}</em></span>`).join(" · ")}</p></section>`}
  function renderSimpleHandout(t,leader=false){
    const one=t.session1,two=t.session2,a=two.activity,decisionFlow=t.contentQualityVersion==="quality-v2"&&one.quickVote&&two.thinkHarder,self=t.facilitationVersion===Simple.SELF_RUNNING_VERSION&&t.facilitation,rounds=self?.rounds;
    const timelineNames={story:"STORY",easy:"EASY",real:"REAL",activity:"JURY VERDICT",practice:"SAY IT KINDLY",wrap:"WRAP"};
    const setup=self?`<section class="simple-section simple-self-setup"><div class="simple-timeline"><span>SETUP <b>${self.setup.minutes}'</b></span>${Object.entries(rounds).map(([key,item])=>`<span>${esc(timelineNames[key]||key.toUpperCase())} <b>${item.minutes}'</b></span>`).join("")}<span>BUFFER <b>${self.transitionBufferMinutes}'</b></span></div><div class="simple-golden-rule"><strong>${esc(self.setup.goldenRuleEn)}</strong><span lang="ko">${esc(self.setup.goldenRuleKo)}</span></div><div class="simple-setup-grid"><p><strong>${esc(self.setup.instructionEn)}</strong><span lang="ko">${esc(self.setup.instructionKo)}</span></p><p><strong>${esc(self.setup.hostRuleEn)}</strong><span lang="ko">${esc(self.setup.hostRuleKo)}</span></p></div></section>`:"";
    const story=`<section class="simple-section simple-story">${self?selfRoundTitle(one.story.heading,rounds.story):`<h2>${esc(one.story.heading)}</h2>`}<div class="simple-story-copy"><p>${one.story.en.map(esc).join(" ")}</p>${!self?`<p lang="ko">${one.story.ko.map(esc).join(" ")}</p>`:""}</div>${leader&&!self?simpleLeader(t.leader.story):""}</section>`;
    const keyWords=self?simpleKeyWords(t):"";
    const easy=`<section class="simple-section">${self?selfRoundTitle("EASY TALK",rounds.easy):"<h2>EASY TALK</h2>"}${simpleQuestions(one.easyTalk,!self)}${leader&&!self?simpleLeader(`${t.leader.easyTalk} ${(t.leader.easyTalkFollowups||[]).join(" · ")}`):""}</section>`;
    const real=`<section class="simple-section">${self?selfRoundTitle("REAL TALK",rounds.real):"<h2>REAL TALK</h2>"}${self?`<p class="simple-culture-swap"><strong>${esc(self.cultureSwapEn)}</strong></p>`:""}${simpleQuestions(one.realTalk,!self)}${leader&&!self?simpleLeader(`${t.leader.realTalk} ${(t.leader.realTalkFollowups||[]).join(" · ")}`):""}</section>`;
    const expressionBody=`<h2>TODAY’S ENGLISH</h2><div>${one.expressions.map(item=>`<p><strong>${esc(item.en)}</strong>${!self?`<span>${esc(item.ko)}</span>`:""}</p>`).join("")}</div>`;
    const expressions=self?`<div class="simple-english">${expressionBody}</div>`:`<section class="simple-section simple-english">${expressionBody}</section>`;
    const quickVote=decisionFlow?`<section class="simple-section simple-reset"><h2>QUICK VOTE · FINGERS 1–2–3</h2><strong>${esc(one.quickVote.en)}</strong>${!self?`<span>${esc(one.quickVote.ko)}</span>`:""}<div class="simple-vote-options">${one.quickVote.options.map((option,index)=>`<b><i>${index+1}</i>${esc(option)}</b>`).join(self?"":" · ")}</div>${!self?`<small>${esc(one.quickVote.noReasonKo)}</small>`:""}</section>`:"";
    const legacyReset=!decisionFlow?`<section class="simple-section simple-reset"><h2>RESET</h2><strong>${esc(two.reset.en)}</strong><span>${esc(two.reset.ko)}</span></section>`:"";
    const sourceMaterial=a.materials[0],reviewMaterials=a.materials.slice(1,5),materials=`${sourceMaterial?`<article class="simple-source-material"><p>${esc(sourceMaterial.en)}</p><span>${esc(sourceMaterial.ko)}</span></article>`:""}<div class="simple-materials${self?" simple-private-cards":""}">${reviewMaterials.map((item,index)=>`<article><b>${self?String.fromCharCode(65+index):index+1}</b><p>${esc(item.en)}</p><span>${esc(item.ko)}</span></article>`).join("")}</div>`;
    ...
    const gap="";
    const stepMinutes=a.stepMinutes||[5,3,10,4,3];
    const selfSteps=["Take one card and retell it in your own words.","Vote at the same time.","Hear the minority view and ask one question.","Decide whether to change your vote. Write the verdict.","Answer Think Harder. (Optional)"];
    const activity=`<section class="simple-section simple-activity">${self?selfRoundTitle("JURY VERDICT",rounds.activity):"<h2>TODAY’S ACTIVITY</h2>"}${!self?`<h3>${esc(a.name)}</h3><p class="simple-instruction">${esc(a.instructionKo)}</p>`:""}${gap}${self?(sourceMaterial?`<article class="simple-source-material"><p>${esc(sourceMaterial.en)}</p></article>`:""):materials}${roles}<ol class="simple-steps">${(self?selfSteps:a.stepsKo).map((step,index)=>`<li>${self?`<small>${stepMinutes[index]} MIN</small>`:""}${esc(step)}</li>`).join("")}</ol><div class="simple-phrases">${a.phrases.map(line=>`<span>${esc(line)}</span>`).join("")}</div><p class="simple-participation">${esc(self?"Everyone speaks once. Listen, then ask a question or add a reason.":a.participationKo)}</p>${leader&&!self?simpleLeader(activityNote):""}</section>`;
    const result=`<section class="simple-section simple-result"><h2>GROUP RESULT</h2><strong>${esc(two.groupResult.en)}</strong>${!self?`<span>${esc(two.groupResult.ko)}</span>`:""}${self?`<p class="simple-verdict-frame">${esc(self.verdictTemplateEn)}</p>`:""}<i></i></section>`;
    const thinkHarder=decisionFlow?`<section class="simple-section simple-think"><h2>THINK HARDER</h2><strong>${esc(two.thinkHarder.en)}</strong>${!self?`<span>${esc(two.thinkHarder.ko)}</span>`:""}</section>`:"";
    const closing=self?`<section class="simple-section simple-practice">${selfRoundTitle("SAY IT KINDLY",rounds.practice)}<strong>${esc(self.practice.promptEn)}</strong><p class="simple-kind-check"><b>□ KIND</b><b>□ HONEST</b></p></section>`:"";
    const final=`<section class="simple-section simple-final">${self?selfRoundTitle("WRAP",rounds.wrap):"<h2>FINAL QUESTION</h2>"}<strong>${esc(two.finalQuestion.en)}</strong>${!self?`<span>${esc(two.finalQuestion.ko)}</span>`:""}${self?`<p>${esc(self.wrap.stemEn)}</p>`:""}${leader&&!self?simpleLeader(t.leader.final):""}</section>`;
    const storyTranslation=self?`<div class="simple-story-translation"><h2>STORY 해석</h2><p lang="ko">${one.story.ko.map(esc).join(" ")}</p></div>`:"";
    const operatingHelp=self?`<section class="simple-section simple-operating-help"><h2>STUCK?</h2><p>${self.stuck.map(esc).join(" · ")}</p><strong>RUNNING LATE</strong><div class="simple-running-late"><p>${esc(self.runningLateEn)}</p><p class="simple-running-late-ko" lang="ko">${esc(self.runningLateKo)}</p></div>${leader?`<small class="simple-leader-note">LEADER NOTE · Confirm the host, timer, and card handout before class.</small>`:""}</section>`:"";
    const emergency=leader&&!self?`<footer class="simple-emergency">${t.leader.emergency.map(line=>`<span>${esc(line)}</span>`).join("")}<b>${esc(t.leader.timeCutKo)}</b></footer>`:"";
  ...
  function renderTopicV4Handout(t){
    const one=t.page1||{},two=t.page2||{},questionList=(items,bilingual=false)=>`<ol class="topic-v4-questions">${(items||[]).map(item=>`<li><strong>${esc(typeof item==="string"?item:item.en)}</strong>${bilingual?`<span>${esc(item.ko)}</span>`:""}</li>`).join("")}</ol>`;
    const words=(items,className="")=>`<div class="topic-v4-words ${className}">${(items||[]).map(item=>`<span><strong>${esc(item.en)}</strong><small>${esc(item.ko)}</small></span>`).join("")}</div>`;
    const story=`<section class="topic-v4-story-panel"><h2>TODAY'S STORY</h2><p class="topic-v4-action">Read aloud. Vote without reasons.</p><p class="topic-v4-story">${esc(one.story?.en||"")}</p><h3>KEY WORDS</h3>${words(one.keyWords,"topic-v4-keywords")}<div class="topic-v4-quick-vote"><h3>QUICK VOTE</h3><div><b>${esc(one.quickVote?.question||"")}</b><div class="topic-v4-vote">${(one.quickVote?.options||[]).map((option,index)=>`<span><i>${index+1}</i>${esc(option)}</span>`).join("")}</div></div></div></section>`;
    const talk=(title,action,items,extra="")=>`<section class="topic-v4-talk"><h2>${title}</h2><p class="topic-v4-action">${action}</p>${extra}${questionList(items)}</section>`;
    const expressions=`<section class="topic-v4-english"><h2>TODAY'S ENGLISH</h2><div class="topic-v4-expressions">${(one.todayEnglish||[]).map(item=>`<p><strong>${esc(item.en)}</strong><span>${esc(item.ko)}</span></p>`).join("")}</div></section>`;
    const translation=`<section class="topic-v4-translation-block"><h2>STORY 해석</h2><p>${esc(one.story?.ko||"")}</p></section>`;
    const pageOne=`<section class="a4-page topic-v4-page-one"><header class="topic-v4-header"><b>THEBOX</b><span>${esc(t.category?.en||"")}</span><em>${esc(t.date)}</em></header><h1 class="topic-v4-main-title">${esc(t.title?.en||"")}</h1><p class="topic-v4-seat-rule"><strong>Closest birthday is #1. Continue clockwise.</strong><span>생일이 오늘과 가장 가까운 사람이 #1입니다. 이후 시계 방향으로 번호를 정하세요.</span></p>${story}<div class="topic-v4-talk-grid">${talk("EASY TALK","Ask & Pass. Pick the next speaker.",one.easyTalk)}${talk("REAL TALK","Ask & Pass. Reveal one number together.",one.realTalk,'<p class="topic-v4-culture">CULTURE SWAP · Start with ‘In my country or workplace…’</p>')}</div>${expressions}${translation}</section>`;
    const pageTwoRule=`<p class="topic-v4-seat-rule"><strong>New table, new people. Re-number 1–N by shoe size — smallest goes first.</strong><span>→ 그룹이 바뀌었으니 신발 사이즈로 새로 번호를 정하세요. 가장 작은 사람이 1번.</span></p>`;
    const randomQuestions=`<section class="topic-v4-wide-section"><h2>RANDOM QUESTIONS</h2><div><p class="topic-v4-action">Ask & Pass. Pick the next speaker.</p>${questionList(two.randomQuestions,true)}</div></section>`;
    const randomWords=`<section class="topic-v4-wide-section topic-v4-random-section"><h2>RANDOM WORDS</h2><div><p class="topic-v4-action">Pick any word. Say what it makes you think of, and why.</p>${words(two.randomWords,"topic-v4-random-words")}</div></section>`;
    const pageTwo=`<section class="a4-page topic-v4-page-two"><time>${esc(t.date)}</time><h1>Round Two</h1>${pageTwoRule}${randomQuestions}${randomWords}</section>`;
    return `<article class="a4-topic topic-v4-handout" data-print-topic="${esc(t.date)}" data-generation-engine="${TopicV4.VERSION}" data-template-version="${TopicV4.TEMPLATE_VERSION}">${pageOne}${pageTwo}</article>`;
  }
  function renderHandout(t,leader=false){
    ...
    return `<article class="a4-topic" data-print-topic="${esc(t.date)}">
  ...
  function renderConversationHandout(t,leader=false){
    ...
    return `<article class="a4-topic conversation-handout v4-handout ${t.topicMode==="context"?"context-handout":""} ${one.format==="evidenceRounds"?"evidence-handout":""} ${leader?"leader-handout":""}" data-print-topic="${esc(t.date)}" data-talkflow-standard="v2" data-template-version="${leader?STANDARD.leaderTemplate:STANDARD.studentTemplate}">
  ...
  function renderPrintCollection(){
    const items=[topics[activeDate]].filter(Boolean);
    ...
    document.title=`${items[0].date}_TheBox_Topic_${safeFilename(items[0].title.ko)}`;
    const canConfirm=items[0].quality?.status==="approved"&&!["checked","printed"].includes(items[0].operatorStatus?.printStatus);
    const pagesPerTopic=2,roleLabel="오늘의 토픽";
    const pages=items.map(topic=>renderHandout(topic,false)).join("");
    return `<div class="print-toolbar"><button class="button secondary" data-action="calendar">← 월 목록으로</button><span>${roleLabel} · ${pagesPerTopic}페이지</span>${canConfirm?`<button class="button secondary" data-action="confirm-print">A4 확인 완료</button>`:""}<button class="button primary" data-action="print-now">PDF 저장 / 인쇄</button></div><div class="paper-stack">${pages}</div>`;
  }
  ...
  function failedSection(failure){if(failure?.type==="structured_content_error"||failure?.error?.type==="structured_content_error"||typeof failure?.candidate?.session1==="string"||typeof failure?.candidate?.session2==="string")return"content";const location=String(failure?.issues?.[0]?.location||failure?.issues?.[0]||failure?.stage||"content");if(location.includes("story"))return"story";if(location.includes("easyTalk"))return"easyTalk";if(location.includes("realTalk"))return"realTalk";if(location.includes("expressions"))return"expressions";if(location.includes("activity")||location.includes("groupResult")||location.includes("thinkHarder")||location.includes("finalQuestion"))return"activity";return failure?.stage==="plan"?"plan":"content"}
  function generationBlocked(t,detailed=false){
    const interrupted=t.operatorStatus?.generationStatus==="interrupted",failed=t.operatorStatus?.generationStatus==="failed"||interrupted,title=interrupted?"GENERATION INTERRUPTED":failed?"GENERATION FAILED":"LEGACY OR INVALID DRAFT",failure=t.generationFailure||{},section=failedSection(failure),source=t.originalDraft||t,issues=(failure.issues||[]).map(item=>typeof item==="string"?{location:section,message:item}:item),bilingualFailure=issues.some(item=>item.id==="Q7"&&String(item.location||"").includes("session1.story")),participationFailure=issues.some(item=>String(item.location||"")==="session2.activity.participationKo"),malformedField=failure.malformedField||failure.error?.malformedField||(["session1","session2"].find(field=>typeof failure.candidate?.[field]==="string")||""),failureType=failure.error?.type||failure.type||"",structuralFailure=failureType==="structured_content_error"||Boolean(malformedField),incompleteFailure=failureType==="incomplete_response",refusalFailure=failureType==="refusal",displayTitle=incompleteFailure?"본문 생성이 중간에 끝났습니다.":refusalFailure?"이 주제로는 내용을 생성할 수 없습니다.":structuralFailure?"생성된 내용의 형식을 확인해야 합니다.":bilingualFailure?"내용 확인 필요":participationFailure?"활동 참여 단계 확인":title;
  ...
  function bindDynamic(){
    ...
    document.querySelectorAll("[data-prepare-mode]").forEach(button=>button.onclick=()=>navigateTo({section:"topics",view:button.dataset.prepareMode,month:monthValue()}));
    document.querySelectorAll("[data-month-filter]").forEach(button=>button.onclick=()=>{prepareFilter=button.dataset.monthFilter;navigateTo(routeFromState(),"replace")});
    document.querySelectorAll("[data-show-missing]").forEach(button=>button.onclick=()=>{prepareFilter="missing";navigateTo({section:"topics",view:"month",month:monthValue()})});
    ...
    document.querySelectorAll("[data-preview-page]").forEach(button=>button.onclick=()=>navigateTo({section:"topics",view:prepareMode,month:monthValue(),date:activeDate,tab:"topic",page:button.dataset.previewPage==="all"?"1":button.dataset.previewPage}));
    ...
    if(operatorTopic&&operatorCreate){operatorCreate.disabled=!operatorTopic.value.trim()||!aiReady();operatorTopic.oninput=()=>{operatorCreate.disabled=!operatorTopic.value.trim()||!aiReady()}}
    document.querySelectorAll("[data-open]").forEach(b=>b.onclick=()=>{const[date,target]=b.dataset.open.split(":");if(target==="print"&&!canPreviewTopic(topics[date])){notify("완성되지 않은 토픽은 미리보기·PDF를 사용할 수 없습니다.",true);return}activeDate=date;printDates.clear();if(target==="print"){navigateTo({section:"print",date});return}const reviewEditor=target==="admin";navigateTo({section:"topics",view:prepareMode,month:date.slice(0,7),date,tab:target==="topic"?"topic":"review",page:target==="topic"?"1":undefined});if(reviewEditor){view="admin";drawerDate="";render()}});
    ...
    document.querySelectorAll("[data-auto-date]").forEach(b=>{b.disabled=!aiReady();b.title=aiReady()?"":aiServerState.configured?"관리자 로그인이 필요합니다.":"AI 서버 설정이 필요합니다.";b.onclick=()=>openCustomTopic(b.dataset.autoDate)});
    document.querySelectorAll("[data-custom-date]").forEach(b=>{b.disabled=!aiReady();b.title=aiReady()?"":aiServerState.configured?"관리자 로그인이 필요합니다.":"AI 서버 설정이 필요합니다.";b.onclick=()=>openCustomTopic(b.dataset.customDate)});document.querySelectorAll("[data-guided-date]").forEach(b=>{b.disabled=!aiReady();b.title=aiReady()?"":aiServerState.configured?"관리자 로그인이 필요합니다.":"AI 서버 설정이 필요합니다.";b.onclick=()=>openCustomTopic(b.dataset.guidedDate,true)});
    document.querySelectorAll("[data-new-topic-date]").forEach(b=>{b.disabled=!aiReady();b.title=aiReady()?"":aiServerState.configured?"관리자 로그인이 필요합니다.":"AI 서버 설정이 필요합니다.";b.onclick=()=>openCustomTopic(b.dataset.newTopicDate)});
    ...
    document.querySelectorAll("[data-publish-topic]").forEach(button=>button.onclick=async()=>{button.disabled=true;try{await publishTopic(button.dataset.publishTopic)}finally{button.disabled=false}});
    ...
    document.querySelectorAll("[data-prev-next]").forEach(b=>b.onclick=()=>{if(b.dataset.prevNext)navigateTo({section:"topics",view:prepareMode,month:b.dataset.prevNext.slice(0,7),date:b.dataset.prevNext,tab:view==="topic"?"topic":"review",page:view==="topic"?previewPage:undefined})});
  ...
  function handleAction(action){
    ...
    if(action==="delete"){if(confirm("이 TheBox Topic 토픽을 삭제할까요?")){delete topics[activeDate];activeDate=Object.keys(topics).sort()[0]||today();saveTopics("삭제했습니다.")}}
    ...
    if(action==="calendar"){printDates.clear();navigateTo({section:"topics",view:prepareMode,month:monthValue()})}
    if(action==="today"){const value=new Date();navigateTo({section:"topics",view:prepareMode,month:`${value.getFullYear()}-${String(value.getMonth()+1).padStart(2,"0")}`})}
    if(action==="previous-month"){const value=new Date(cursor.getFullYear(),cursor.getMonth()-1,1,12);navigateTo({section:"topics",view:prepareMode,month:`${value.getFullYear()}-${String(value.getMonth()+1).padStart(2,"0")}`})}
    if(action==="next-month"){const value=new Date(cursor.getFullYear(),cursor.getMonth()+1,1,12);navigateTo({section:"topics",view:prepareMode,month:`${value.getFullYear()}-${String(value.getMonth()+1).padStart(2,"0")}`})}
    if(action==="auto-compose-month")autoComposeMonth();
    if(action==="settings"){navigateTo({section:"settings",panel:settingsPanel})}
  ...
  async function createConversationTopic(){
    ...
    await generateAndStore({date,generationMode:"guided",topicHint:keyword,conversationDirection:mood,avoid:$("#operator-avoid")?.value.trim()||""});
  }
  function normalizeGenerationRequest(input={}){const topicHint=String(input.topicHint??input.keyword??"").trim(),generationMode=input.generationMode==="auto"?"auto":input.generationMode==="guided"?"guided":topicHint?"guided":"auto";return{date:String(input.date||""),generationMode,topicHint:generationMode==="guided"?topicHint:"",conversationDirection:String(input.conversationDirection??input.mood??"auto"),source:String(input.source||""),avoid:String(input.avoid||""),repairSection:String(input.repairSection||"")}}
  function recentTopicContext(date){const target=new Date(`${date}T12:00:00`);return Object.values(topics).filter(item=>item?.date&&item.date!==date&&Math.abs(target-new Date(`${item.date}T12:00:00`))<=60*86400000).sort((a,b)=>b.date.localeCompare(a.date)).slice(0,20).map(item=>({date:item.date,title:String(item.title?.ko||item.title?.en||""),category:String(item.category?.ko||item.category?.en||""),activityType:String(item.session2?.activity?.name||item.midGame?.name||"")}))}
  ...
  async function regenerateSimpleSection(section){
    ...
    try{
      ...
      next.contentQualityVersion=previous.contentQualityVersion||"quality-v1";const evaluation=Simple.evaluate(next,Object.values(topics));if(!evaluation.ready)throw Object.assign(new Error("부분 재생성 후 전체 연결성 검사를 통과하지 못했습니다."),{issues:evaluation.issues});next.contentQuality=evaluation.quality;next.quality={status:"review",score:evaluation.quality.total,issues:[]};next.operatorStatus={...next.operatorStatus,reviewStatus:"review",printStatus:"unchecked"};next.updatedAt=new Date().toISOString();topics[activeDate]=next;dirty=true;saveTopics("선택한 부분만 다시 생성하고 전체 연결성을 확인했습니다.");
    }catch(error){notify(`부분 재생성 실패: ${error.message}`,true)}
  }
  function setTopicCreateMode(guided=false){const autoPanel=$(".topic-create-default"),panel=$("#guided-topic-fields"),button=$("#show-guided-topic"),error=$("#custom-keyword-error");autoPanel.hidden=guided;panel.hidden=!guided;button.setAttribute("aria-expanded",String(guided));error.hidden=true;$("#custom-keyword").removeAttribute("aria-invalid");if(guided)$("#custom-keyword").focus()}
  function openCustomTopic(date,guided=false){
    $("#custom-date").value=date;$("#custom-keyword").value="";$("#custom-source").value="";$("#custom-avoid").value="";$("#custom-mood").value="auto";$("#topic-create-options").open=false;setTopicCreateMode(guided);$("#custom-topic-dialog").showModal();
  }
  async function autoComposeMonth(){
    ...
    for(let index=0;index<targets.length;index++)await generateAndStore({date:targets[index],generationMode:"auto",conversationDirection:"auto"});
  ...
  async function publishTopic(date){
    const topic=topics[date];
    if(!topic||topic.quality?.status!=="approved"){notify("승인된 토픽만 멤버에게 공개할 수 있습니다.",true);return}
    const response=await fetch("/api/talkflow/published-topics",{method:"PUT",credentials:"same-origin",headers:{"content-type":"application/json","x-talkflow-request":"app"},body:JSON.stringify({topic})});
    const payload=await response.json().catch(()=>({}));
    if(!response.ok){notify(payload.error?.message||"공유 토픽 저장에 실패했습니다.",true);return}
    topic.publishedAt=payload.publishedAt;saveTopics("멤버 화면에 오늘의 토픽을 공개했습니다.");
  }
  ...
  async function regenerate(section){
    if(!aiReady()){notify(aiServerState.configured?"관리자 로그인이 필요합니다. 작성된 내용은 그대로 보존했습니다.":"AI 서버 설정이 필요합니다. 작성된 내용은 그대로 보존했습니다.",true);return}
    ...
    const topicContext={title:topic.title,category:topic.category,topicMode:topic.topicMode,target};
    const prompt=window.TalkFlowLegacyRepairPrompt.build(scope,topicContext);
    try{
      preserveVersion(topic);
      const modelId=await preflightModel(),payload=await AiClient.generate({operation:"legacy_repair",model:modelId,max_tokens:6000,scope,topicContext,prompt});
  ...
  function importJson(file){
    const reader=new FileReader();reader.onload=()=>{
      try{
        const parsed=JSON.parse(reader.result),incoming=parsed.topics||parsed,next=mergeIncomingTopics(incoming);
  ...
  function viewerHtml(data){
    const normalized=Object.fromEntries(Object.entries(data).filter(([date])=>/^\d{4}-\d{2}-\d{2}$/.test(date)).map(([date,topic])=>[date,{...topic,date}])),safe=JSON.stringify(normalized).replace(/</g,"\\u003c");
  ...
  async function gistPush(){
    ...
    try{
      const url=settings.gistId?`https://api.github.com/gists/${settings.gistId}`:"https://api.github.com/gists",response=await fetch(url,{method:settings.gistId?"PATCH":"POST",headers:{Authorization:`Bearer ${settings.gistToken}`,"Content-Type":"application/json","X-GitHub-Api-Version":"2022-11-28"},body:JSON.stringify({description:`TheBox Topic ${monthPrefix()}`,public:false,files})});
      const payload=await response.json();if(!response.ok)throw new Error(payload.message||`HTTP ${response.status}`);settings.gistId=payload.id;settings.lastShareUrl=`https://gist.githack.com/${payload.owner?.login||"anonymous"}/${payload.id}/raw/${monthFile("viewer")}`;saveSettings();navigator.clipboard?.writeText(settings.lastShareUrl).catch(()=>{});notify("TheBox Topic 전용 Gist 저장 및 공개 링크 복사를 완료했습니다.");
  ...
  async function gistPull(){
    ...
    try{
      ...
      const file=payload.files?.[monthFile()];if(!file)throw new Error(`${monthFile()} 파일이 없습니다.`);const parsed=JSON.parse(file.content);topics=mergeIncomingTopics(parsed.topics);settings.gistId=id;saveSettings();saveTopics("TheBox Topic 전용 Gist에서 불러왔습니다.");
    }catch(error){notify(`Gist 불러오기 실패: ${error.message}`,true)}
  }
  function saveSettings(){const persisted={...settings};delete persisted.apiKey;localStorage.setItem(KEYS.settings,JSON.stringify(persisted))}
  ...
  function showModelDiagnostic(error){const details=$("#model-diagnostics"),safe=safeDiagnostics(error);details.hidden=false;details.querySelector("pre").textContent=JSON.stringify(safe)}
  function showAuthenticationError(error){$("#ai-connection-status").textContent=error?.type==="server_not_configured"?"서버 설정 필요":"로그인 필요";$("#api-key-guidance").hidden=false;showModelDiagnostic(error)}
  async function refreshModelsFromSettings(){
    settings={...settings,connectionStatus:"unchecked"};try{await fetchAvailableModels();renderModelControls();$("#ai-connection-status").textContent="사용 가능한 모델을 확인했습니다.";$("#api-key-guidance").hidden=true;$("#model-diagnostics").hidden=true}catch(error){if([401,403,503].includes(error.httpStatus)||["login_required","server_not_configured"].includes(error.type))showAuthenticationError(error);else{$("#ai-connection-status").textContent=`모델 목록 연결 실패${error.httpStatus?` · HTTP ${error.httpStatus}`:""} · ${error.type||"unknown_error"}`;showModelDiagnostic(error)}}
  }
  ...
  async function testAiConnection(){
    ...
    try{
      const modelId=await preflightModel({signal:controller.signal}),payload=await AiClient.generate({model:modelId,max_tokens:8,messages:[{role:"user",content:"Reply with OK."}]},{signal:controller.signal});
      if(!Array.isArray(payload.content))throw Object.assign(new Error("응답 형식 오류"),{type:"response_format_error",stage:"connection",modelId});
  ...
  function renderServerControls(){
    const badge=$("#ai-server-badge"),guidance=$("#ai-server-guidance"),login=$("#admin-login-form"),logout=$("#admin-logout");
    try{legacyApiKeyPresent=legacyApiKeyPresent||Boolean(JSON.parse(localStorage.getItem(KEYS.settings)||"{}").apiKey)}catch{}
    badge.textContent=!aiServerState.checked?"확인 중":!aiServerState.configured?"설정 필요":aiServerState.authenticated?"정상":"로그인 필요";
    guidance.textContent=!aiServerState.checked?"서버 연결 상태를 확인하고 있습니다.":!aiServerState.configured?"서버 환경변수를 설정하면 AI 생성을 사용할 수 있습니다.":aiServerState.authenticated?`서버 연결 정상 · ${aiServerState.model}`:"관리자 로그인 후 AI 생성을 사용할 수 있습니다.";
    login.hidden=!aiServerState.checked||!aiServerState.configured||aiServerState.authenticated;logout.hidden=!aiServerState.authenticated;
    $("#legacy-key-migration").hidden=!legacyApiKeyPresent;$("#refresh-models").disabled=!aiReady();$("#test-ai-connection").disabled=!aiReady();
  }
  async function refreshServerStatus(){try{aiServerState={checked:true,...await AiClient.status()}}catch{aiServerState={checked:true,configured:false,authenticated:false,model:"Claude Sonnet 4.6"}}renderServerControls();render()}

  document.querySelectorAll(".tab[data-view]").forEach(button=>button.onclick=()=>{if(!confirmDirty())return;navigateTo({section:"topics",view:"tasks",month:monthValue()})});
  ...
  function openSettingsDialog(push=true){
    ...
    renderServerControls();$("#admin-password").value="";$("#gist-token").value="";$("#gist-token").placeholder=settings.gistToken?"Gist token이 저장되어 있습니다.":"gist 권한 토큰";$("#gist-id").value="";$("#gist-id").placeholder=settings.gistId?"Gist ID가 저장되어 있습니다.":"gist 파일 ID";$("#print-backgrounds").checked=settings.printBackgrounds!==false;
    ...
    $("#exclude-public-holidays").checked=Boolean(settings.excludePublicHolidays);renderModelControls();renderServerControls();
  ...
  $("#settings-button").onclick=()=>openSettingsDialog(true);
  $("#settings-form").addEventListener("submit",event=>{
    ...
    settings={...settings,anthropicModel:$("#anthropic-model").value||settings.anthropicModel||AnthropicModels.DEFAULT_MODEL,gistToken:$("#gist-token").value.trim()||settings.gistToken||"",gistId:$("#gist-id").value.trim()||settings.gistId||"",operatingWeekdays,excludedDates,additionalDates,excludePublicHolidays,printBackgrounds:$("#print-backgrounds").checked,lastSchedule:{operatingWeekdays,excludedDates,additionalDates,excludePublicHolidays}};
    saveSettings();notify("TheBox Topic 설정을 저장했습니다.");render()
  });
  $("#admin-login").onclick=async()=>{const password=$("#admin-password").value,button=$("#admin-login");if(!password)return;button.disabled=true;try{await AiClient.login(password);$("#admin-password").value="";await refreshServerStatus();notify("관리자 로그인이 완료됐습니다.")}catch(error){showAuthenticationError(error)}finally{button.disabled=false}};
  $("#admin-logout").onclick=async()=>{try{await AiClient.logout()}finally{settings={...settings,connectionStatus:"unchecked"};saveSettings();await refreshServerStatus()}};
  $("#remove-legacy-api-key").onclick=()=>{try{const value=JSON.parse(localStorage.getItem(KEYS.settings)||"{}");delete value.apiKey;localStorage.setItem(KEYS.settings,JSON.stringify(value));legacyApiKeyPresent=false;renderServerControls();notify("이전 브라우저 API 키를 삭제했습니다.")}catch{notify("이전 API 키를 삭제하지 못했습니다.",true)}};
  $("#refresh-models").onclick=startModelsRefresh;$("#test-ai-connection").onclick=startConnectionTest;$("#anthropic-model").onchange=event=>{settings={...settings,anthropicModel:event.target.value,modelFallback:null,connectionStatus:"unchecked"};saveSettings();renderModelControls()};
  document.querySelectorAll("[data-settings-section]").forEach(button=>button.onclick=()=>{settingsPanel=button.dataset.settingsSection;navigateTo({section:"settings",panel:settingsPanel})});
  ...
  $("#show-guided-topic").onclick=()=>setTopicCreateMode(true);$("#use-auto-topic").onclick=()=>setTopicCreateMode(false);$("#custom-keyword").oninput=()=>{if($("#custom-keyword").value.trim()){$("#custom-keyword-error").hidden=true;$("#custom-keyword").removeAttribute("aria-invalid")}};
  $("#custom-topic-form").addEventListener("submit",async event=>{
    if(event.submitter?.value==="cancel")return;
    event.preventDefault();
    const date=$("#custom-date").value,generationMode=event.submitter?.value==="guided"?"guided":"auto",topicHint=$("#custom-keyword").value.trim(),source=$("#custom-source").value.trim(),conversationDirection=$("#custom-mood").value;
    if(!date){notify("날짜를 확인해 주세요.",true);return}
    if(generationMode==="guided"&&!topicHint){const error=$("#custom-keyword-error"),input=$("#custom-keyword");error.hidden=false;input.setAttribute("aria-invalid","true");input.focus();return}
    if(topics[date]){notify("선택한 날짜에 토픽이 있습니다.",true);return}
    const submitter=event.submitter;submitter.disabled=true;try{await generateAndStore({date,generationMode,topicHint:generationMode==="guided"?topicHint:"",conversationDirection,source,avoid:$("#custom-avoid").value.trim()});$("#custom-topic-dialog").close()}finally{submitter.disabled=false}
  });
