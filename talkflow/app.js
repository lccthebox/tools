...
function canPreviewTopic(topic){if(!topic||["running","failed","interrupted"].includes(topic.operatorStatus?.generationStatus))return false;return!topic.generatedConversation||topic.generationEngine===Simple.VERSION&&Simple.evaluate(topic,Object.values(topics)).ready||topic.generationEngine===Generation.VERSION&&Generation.evaluate(topic,Object.values(topics)).ready}
  ...
  function render(){
    ...
    renderList();
    $(".app-shell").classList.toggle("wide-mode",view==="prepare"||view==="batch"||view==="print");
    $("#calendar-view").innerHTML=view==="prepare"?renderPreparationWorkspace():view==="batch"?renderBatchWorkspace():"";
    ...
    $("#calendar-view").hidden=!['prepare','batch'].includes(view);
    $("#student-view").hidden=view!=="student";$("#leader-view").hidden=view!=="leader";$("#admin-view").hidden=view!=="admin";
    $("#print-view").hidden=view!=="print";
    document.querySelectorAll(".tab").forEach(button=>{const active=button.dataset.view==="prepare"&&view==="prepare"||button.dataset.view==="batch"&&["batch","print"].includes(view);button.classList.toggle("is-active",active);button.setAttribute("aria-current",active?"page":"false")});
    updateDocumentTitle();
    bindDynamic();
  }
  function monthValue(){return monthPrefix()}
  function routeFromState(){
    if(view==="print")return{section:"print",date:activeDate,role:printLeader?"leader":"student"};
    if(view==="batch")return{section:"print"};
    ...
    const date=drawerDate||(["admin","student","leader"].includes(view)?activeDate:"");
    if(date){route.date=date;route.tab=view==="student"?"student":view==="leader"?"leader":"review";if(["student","leader"].includes(route.tab))route.page=previewPage}
    return route;
  }
  ...
  function updateDocumentTitle(){const topic=topics[activeDate],month=`${cursor.getMonth()+1}월`,singlePrint=view==="print"&&printDates.size===1&&topic,section=new URLSearchParams(location.search).get("section"),label=section==="settings"?"설정":view==="student"?`${topic?.title?.ko||"토픽"} · 학생용`:view==="leader"?`${topic?.title?.ko||"토픽"} · 리더용`:singlePrint?`${topic.date}_TheBox_TalkFlow_${topic.title?.ko||topic.title?.en||"topic"}`:view==="batch"||view==="print"?"일괄 인쇄":drawerDate?`${topic?.title?.ko||drawerDate} · 내용 검수`:prepareMode==="month"?`${month} 전체 월`:prepareMode==="calendar"?`${month} 달력`:"해야 할 일";document.title=`${label} · TheBox Talk Flow`}
  function renderList(){
  ...
  function primaryAction(date,topic){const state=lifecycleState(topic),labels={"generation-failed":"문제 해결",review:"확인하기","print-ready":"승인하기",empty:"토픽 만들기",approved:"PDF 보기",invalid:"이전 형식 보기",draft:"진행 보기"};if(!topic)return `<button class="button primary" data-custom-date="${date}">${labels.empty}</button>`;if(topic.operatorStatus?.generationStatus==="interrupted")return `<button class="button primary" data-action="regenerate-v2">다시 생성</button><button class="button secondary" data-open="${date}:admin">초안 보기</button>`;const target=state.key==="approved"?"student":"admin";return `<button class="button ${state.key==="approved"?"secondary":"primary"}" data-open="${date}:${target}">${labels[state.key]||"확인하기"}</button>`}
  ...
  function renderSimpleHandout(t,leader=false){
    const one=t.session1,two=t.session2,a=two.activity,decisionFlow=t.contentQualityVersion==="quality-v2"&&one.quickVote&&two.thinkHarder;
    ...
    const quickVote=decisionFlow?`<section class="simple-section simple-reset"><h2>QUICK VOTE</h2><strong>${esc(one.quickVote.en)}</strong><span>${esc(one.quickVote.ko)}</span><div class="simple-vote-options">${one.quickVote.options.map(option=>`<b>□ ${esc(option)}</b>`).join("")}</div><small>${esc(one.quickVote.noReasonKo)}</small></section>`:"";
    ...
    const sourceMaterial=a.materials[0],reviewMaterials=a.materials.slice(1,5),materials=`${sourceMaterial?`<article class="simple-source-material"><p>${esc(sourceMaterial.en)}</p><span>${esc(sourceMaterial.ko)}</span></article>`:""}<div class="simple-materials">${reviewMaterials.map((item,index)=>`<article><b>${index+1}</b><p>${esc(item.en)}</p><span>${esc(item.ko)}</span></article>`).join("")}</div>`;
    ...
    const thinkHarder=decisionFlow?`<section class="simple-section simple-think"><h2>THINK HARDER</h2><strong>${esc(two.thinkHarder.en)}</strong><span>${esc(two.thinkHarder.ko)}</span></section>`:"";
    ...
    return `<article class="a4-topic simple-handout${leader?" simple-leader-handout":""}" data-print-topic="${esc(t.date)}" data-generation-engine="${Simple.VERSION}" data-template-version="${Simple.TEMPLATE_VERSION}">${page(1,story+easy+real+expressions+quickVote)}${page(2,legacyReset+activity+result+thinkHarder+final)}</article>`;
  }
  ...
  function dailyNav(t,mode){
    ...
    const pageSwitch=["student","leader"].includes(mode)?`<div class="route-page-switch" aria-label="미리보기 페이지"><button class="${previewPage==="1"?"is-active":""}" data-preview-page="1">1페이지</button><button class="${previewPage==="2"?"is-active":""}" data-preview-page="2">2페이지</button></div>`:"";
    return `<div class="daily-nav"><button data-action="calendar">← 토픽 목록</button><div><strong>${t.date} · ${weekday(t.date)}</strong><span>${lifecycleState(t).label} · 마지막 저장 ${new Date(t.updatedAt||t.createdAt).toLocaleString("ko-KR")}</span>${t.generatedConversation&&t.generationEngine!==Simple.VERSION?'<small class="legacy-format-badge">이전 형식</small>':""}</div><nav class="topic-tabs" aria-label="선택한 토픽"><button class="${mode==="admin"?"is-active":""}" data-open="${t.date}:admin">내용 검수</button><button class="${mode==="student"?"is-active":""}" data-open="${t.date}:student" ${canPreviewTopic(t)?"":"disabled"}>학생용</button><button class="${mode==="leader"?"is-active":""}" data-open="${t.date}:leader" ${canPreviewTopic(t)?"":"disabled"}>리더용</button></nav>${pageSwitch}<button data-prev-next="${dates[index-1]||""}" ${index<=0?"disabled":""} aria-label="이전 날짜">←</button><button data-prev-next="${dates[index+1]||""}" ${index<0||index>=dates.length-1?"disabled":""} aria-label="다음 날짜">→</button></div>`;
  }
  ...
  function bindDynamic(){
    ...
    document.querySelectorAll("[data-prev-next]").forEach(b=>b.onclick=()=>{if(b.dataset.prevNext)navigateTo({section:"topics",view:prepareMode,month:b.dataset.prevNext.slice(0,7),date:b.dataset.prevNext,tab:view==="leader"?"leader":view==="student"?"student":"review",page:["student","leader"].includes(view)?previewPage:undefined})});
    document.querySelectorAll("[data-print-leader]").forEach(b=>b.onclick=()=>{if(!canPreviewTopic(topics[b.dataset.printLeader])){notify("완성되지 않은 토픽은 리더용 PDF를 사용할 수 없습니다.",true);return}printDates=new Set([b.dataset.printLeader]);navigateTo({section:"print",date:b.dataset.printLeader,role:"leader"})});
  ...
  function applyGenerationFailure(pending,request,error,plan=null){
    ...
    const message=error.type==="incomplete_response"?"본문 생성이 중간에 끝났습니다.":error.type==="refusal"?"이 주제로는 내용을 생성할 수 없습니다.":error.type==="structured_content_error"?"생성된 내용의 형식을 확인해야 합니다.":error.factMismatch?"Story 영어와 한국어의 핵심 정보가 일치하지 않아 저장하지 않았습니다.":"토픽 내용을 완성하지 못했습니다. 작성된 초안은 보존했습니다. 실패 이유를 확인하거나 해당 섹션만 다시 생성하세요.";
    saveTopics(message);
  }
  ...
  async function gistPush(){
    ...
    try{
      ...
      const payload=await response.json();if(!response.ok)throw new Error(payload.message||`HTTP ${response.status}`);settings.gistId=payload.id;settings.lastShareUrl=`https://gist.githack.com/${payload.owner?.login||"anonymous"}/${payload.id}/raw/${monthFile("viewer")}`;saveSettings();navigator.clipboard?.writeText(settings.lastShareUrl).catch(()=>{});notify("Talk Flow 전용 Gist 저장 및 공개 링크 복사를 완료했습니다.");
  ...
  async function gistPull(){
    ...
    try{
      ...
      const file=payload.files?.[monthFile()];if(!file)throw new Error(`${monthFile()} 파일이 없습니다.`);const parsed=JSON.parse(file.content);topics=mergeIncomingTopics(parsed.topics);settings.gistId=id;saveSettings();saveTopics("Talk Flow 전용 Gist에서 불러왔습니다.");
  ...
  document.querySelectorAll(".tab[data-view]").forEach(button=>button.onclick=()=>{if(!confirmDirty())return;if(button.dataset.view==="batch")navigateTo({section:"print"});else navigateTo({section:"topics",view:"tasks",month:monthValue()})});
  ...
  function openSettingsDialog(push=true){
    ...
    renderServerControls();$("#admin-password").value="";$("#gist-token").value="";$("#gist-token").placeholder=settings.gistToken?"Gist token이 저장되어 있습니다.":"gist 권한 토큰";$("#gist-id").value="";$("#gist-id").placeholder=settings.gistId?"Gist ID가 저장되어 있습니다.":"gist 파일 ID";$("#default-print-role").value=settings.defaultPrintRole||"student";$("#print-backgrounds").checked=settings.printBackgrounds!==false;
