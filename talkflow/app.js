...
function canPreviewTopic(topic){if(!topic||["running","failed"].includes(topic.operatorStatus?.generationStatus))return false;return!topic.generatedConversation||topic.generationEngine===Simple.VERSION&&Simple.evaluate(topic,Object.values(topics)).ready||topic.generationEngine===Generation.VERSION&&Generation.evaluate(topic,Object.values(topics)).ready}
  ...
  function preparationRow(date,className="task-row"){const topic=topics[date],state=lifecycleState(topic);return `<article class="${className} ${state.key}" data-drawer-date="${date}" tabindex="0"><time><b>${Number(date.slice(8))}일</b><span>${weekday(date)}</span></time><div class="preparation-topic"><strong>${topic?koTitle(topic.title?.ko||topic.title?.en):"아직 토픽이 없습니다"}</strong></div><span class="status-pill ${state.key}">${state.label}</span><div class="row-primary">${primaryAction(date,topic)}</div><details class="row-more"><summary aria-label="추가 작업">⋯</summary><div>${topic?`<button data-open="${date}:admin">내용 검수</button><button data-open="${date}:student">학생용</button><button data-open="${date}:leader">리더용</button>`:`<button data-auto-date="${date}">추천 주제</button>`}</div></details></article>`}
  ...
  function renderTaskHome(){const all=operatingDates(),actionable=all.filter(date=>lifecycleState(topics[date]).key!=="approved").sort(comparePriority),shown=actionable.slice(0,8),missing=all.filter(date=>!topics[date]),week=weekDates(),weekCounts=week.reduce((acc,date)=>{const key=lifecycleState(topics[date]).key;acc[key]=(acc[key]||0)+1;return acc},{}),weekDone=week.length&&week.every(date=>lifecycleState(topics[date]).key==="approved");return `<div class="task-home"><section class="task-section"><header><div><p class="eyebrow">PRIORITY</p><h2>해야 할 일 <span>${actionable.length}건</span></h2></div></header><div class="task-list">${shown.length?shown.map(date=>preparationRow(date)).join(""):`<p class="empty-task">이번 달에 바로 처리할 항목이 없습니다.</p>`}</div>${missing.length>shown.filter(date=>!topics[date]).length?`<button class="text-action" data-show-missing>미작성 ${missing.length}건 모두 보기</button>`:""}</section><section class="this-week"><header><div><p class="eyebrow">THIS WEEK</p><h2>이번 주</h2></div><p>승인 ${weekCounts.approved||0} · 검수 ${(weekCounts.review||0)+(weekCounts['print-ready']||0)} · 미작성 ${weekCounts.empty||0}</p></header>${weekDone?`<p class="week-complete">이번 주 토픽 준비가 모두 끝났습니다.</p>`:`<div class="week-list">${week.map(date=>preparationRow(date,"week-row")).join("")}</div>`}</section></div>`}
  ...
  async function regenerate(section){
    ...
    const prompt=`Create or repair TheBox Talk Flow ${scope} as strict JSON for one mixed-confidence adult group.
<talkflow-standard version="2" student-template="student-v4" leader-template="leader-v4">
