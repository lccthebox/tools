...
function canPreviewTopic(topic){if(!topic||["running","failed","interrupted"].includes(topic.operatorStatus?.generationStatus))return false;return!topic.generatedConversation||topic.generationEngine===TopicV4.VERSION&&TopicV4.evaluate(topic).ready||topic.generationEngine===Simple.VERSION&&Simple.evaluate(topic,Object.values(topics)).ready||topic.generationEngine===Generation.VERSION&&Generation.evaluate(topic,Object.values(topics)).ready}
  ...
  function renderTopicV4Handout(t){
    const one=t.page1||{},two=t.page2||{},header=page=>`<header class="topic-v4-header"><b>THEBOX</b><span>${esc(t.category?.en||"")}</span><em>${esc(t.date)}</em></header>${page===1?`<div class="topic-v4-title"><h1>${esc(t.title?.en||"")}</h1></div>`:`<div class="topic-v4-title topic-v4-round-title"><h1>Round Two</h1></div>`}`,block=(title,body,className="")=>`<section class="topic-v4-block ${className}"><h2>${esc(title)}</h2>${body}</section>`,questions=(items,bilingual=false)=>`<ol class="topic-v4-questions">${(items||[]).map(item=>`<li><strong>${esc(typeof item==="string"?item:item.en)}</strong>${bilingual?`<span>${esc(item.ko)}</span>`:""}</li>`).join("")}</ol>`;
    const story=block("TODAY’S STORY",`<p class="topic-v4-action">Read aloud. Vote without reasons.</p><div class="topic-v4-story"><p>${esc(one.story?.en||"")}</p></div>`,"topic-v4-story-block");
    ...
    const easy=block("EASY TALK",`<p class="topic-v4-action">Ask & Pass. Pick the next speaker.</p>${questions(one.easyTalk)}`);
    const real=block("REAL TALK",`<p class="topic-v4-action">Ask & Pass. Reveal one number together.</p>${questions(one.realTalk)}`);
    const expressions=block("TODAY’S ENGLISH",`<div class="topic-v4-expressions">${(one.todayEnglish||[]).map(item=>`<p><strong>${esc(item.en)}</strong><span>${esc(item.ko)}</span></p>`).join("")}</div>`);
    const translation=block("STORY 해석",`<p class="topic-v4-translation">${esc(one.story?.ko||"")}</p>`,"topic-v4-translation-block");
    const randomQuestions=block("RANDOM QUESTIONS",`<p class="topic-v4-action">Ask & Pass. Pick the next speaker.</p>${questions(two.randomQuestions,true)}`);
    ...
    const pageOneRule=`<p class="topic-v4-seat-rule"><strong>Closest birthday is #1. Continue clockwise.</strong><span>생일이 오늘과 가장 가까운 사람이 1번입니다. 이후 시계 방향으로 번호를 정하세요.</span></p>`;
    ...
    return `<article class="a4-topic topic-v4-handout" data-print-topic="${esc(t.date)}" data-generation-engine="${TopicV4.VERSION}" data-template-version="${TopicV4.TEMPLATE_VERSION}"><section class="a4-page">${header(1)}${pageOneRule}<main class="topic-v4-body">${story}${words}${vote}<div class="topic-v4-talk-grid">${easy}${real}</div>${expressions}${translation}</main></section><section class="a4-page">${header(2)}${pageTwoRule}<main class="topic-v4-body">${randomQuestions}${randomWords}</main></section></article>`;
  }
