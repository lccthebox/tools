...
module.exports=async function handler(request,response){
  ...
  if(!topic||topic.quality?.status!=="approved"||!/^\d{4}-\d{2}-\d{2}$/.test(topic.date||"")||!topic.session1||!topic.session2){json(response,400,{error:{type:"invalid_topic",message:"승인된 완성 토픽만 공개할 수 있습니다."}});return}
